...
from src.escape_rooms.state import GameWorld, PartyState, Storyboard, TickAction
...
class EpisodeResult:
    ...
    # True when the episode ended because all accuse attempts were wrong
    # (mystery themes only — always False when no storyboard is supplied).
    ...
    def run(self, policy, record_history: bool = False) -> EpisodeResult:
        ...
        try:
            while not ps.game_over and ps.tick < self.max_ticks:
                if gp._check_victory(world, ps, storyboard):
                    ps.victory = True
                    break
                if ps.wrong_deduction:
                    break
                ps.tick += 1
                visible = gp._objects_in_room(world, ps)
                action_space = gp._build_action_space(world, ps, visible, storyboard)
                ...
                note, target = gp._resolve_action(action, world, ps, storyboard)
    ...
    def run(self, policy, record_history: bool = False) -> CooperativeEpisodeResult:
        ...
        try:
            while not ps.game_over and ps.tick < self.max_ticks:
                ...
                if ps.victory or ps.wrong_deduction:
                    break
        finally:
            gp._stream = orig_stream
